  ...
  return (
    <div className="bg-white dark:bg-slate-900/60 rounded-[2.5rem] border p-6 sm:p-12 shadow-sm space-y-10 glow-card animate-in slide-in-from-right-4">
      ...
      <div className="space-y-8">
        <div className="p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 space-y-3">
        ...
        {canManage ? (
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button
                  ...
                  className={`flex items-center gap-4 p-5 rounded-[2rem] border transition-all disabled:opacity-60 ${settings.defaultVisibility === 'private' ? 'border-primary bg-primary/5' : 'border-slate-100 dark:border-slate-800'}`}
                >
                ...
                <button
                  ...
                  className={`flex items-center gap-4 p-5 rounded-[2rem] border transition-all disabled:opacity-60 ${settings.defaultVisibility === 'family' ? 'border-primary bg-primary/5' : 'border-slate-100 dark:border-slate-800'}`}
                >
          ...
          <div className="p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/40">
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">{t.settings.vault.memberNoticeTitle}</p>
            <p className="text-xs text-slate-500 mt-2">{t.settings.vault.memberNoticeDesc}</p>
          </div>
        )}
        ...
        <button
          ...
          className="w-full text-left p-6 bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/20 rounded-[2rem] flex items-start gap-4 hover:border-amber-300 transition-all"
        >
          ...
          <div>
            ...
            <div className="mt-2 text-[10px] font-bold text-amber-700 dark:text-amber-300">
              {isHealthLoading
                ? 'Analyzing...'
                : `${healthSummary?.duplicateGroupsCount || 0} groups | ${healthSummary?.duplicateItemsCount || 0} duplicates | ${formatBytes(healthSummary?.reclaimableBytes || 0)} recoverable`}
            </div>
          </div>
        </button>

        <div className="p-6 bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-900/20 rounded-[2rem] space-y-4">
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Select value={selectedTransferMembershipId} onValueChange={onSelectTransferMembership}>
                <SelectTrigger className="flex-1 bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-800 rounded-xl px-4 py-2.5 text-sm">
                  <SelectValue placeholder={t.settings.vault.transfer.selectPlaceholder} />
                </SelectTrigger>
                <SelectContent>
        ...
        <div className="p-6 bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-900/20 rounded-[2rem] space-y-4">
          <div>

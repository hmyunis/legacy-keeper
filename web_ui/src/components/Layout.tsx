...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  const { syncPushSubscriptionIfGranted } = usePushNotifications();
  ...
  }, [isAuthenticated, syncPushSubscriptionIfGranted]);
  ...
  const SidebarContent = ({ isMobile = false }) => (
    <>
      <div className="p-6 flex items-center justify-between gap-3">
        <Link to="/dashboard" onClick={() => isMobile && setMobileMenuOpen(false)} className={`flex items-center gap-3 group/logo transition-all hover:opacity-80 ${(!isSidebarOpen && !isMobile) ? 'justify-center w-full' : ''}`}>
        ...
        {filteredNavigation.map((item) => {
          ...
          return (
            <Link
              ...
              activeProps={{
                className:
                  'bg-primary/15 dark:bg-primary/20 text-primary dark:text-primary border-primary/35 font-semibold shadow-sm before:absolute before:left-0 before:top-2 before:bottom-2 before:w-1 before:rounded-r-full before:bg-primary',
              }}
              className={`group relative flex items-center gap-4 px-3.5 py-3 rounded-xl border border-transparent transition-all text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-800 dark:hover:text-slate-200 ${(!isSidebarOpen && !isMobile) ? 'justify-center px-0' : ''}`}
            >
              <span className={`shrink-0 transition-transform ${activeTab === item.name ? 'text-primary scale-110' : 'text-slate-400'}`}>{item.icon}</span>
      ...
      <div className="p-4 border-t border-slate-50 dark:border-slate-800/50 mt-auto">
        <div className="flex flex-col gap-2">
          <Link 
            ...
            onClick={() => isMobile && setMobileMenuOpen(false)}
            className={`flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all hover:border-primary/30 ${(!isSidebarOpen && !isMobile) ? 'justify-center px-0 bg-transparent border-none shadow-none' : ''}`}
          >
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] dark:bg-slate-950 transition-colors duration-300 overflow-hidden relative">
      ...
      <aside className={`lg:hidden fixed top-0 bottom-0 left-0 w-70 bg-white dark:bg-slate-900 z-50 flex flex-col shadow-2xl transition-transform duration-500 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}><SidebarContent isMobile={true} /></aside>

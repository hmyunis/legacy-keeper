...
import { Camera, Plus, User as UserIcon, Mail, History, Save } from 'lucide-react';
...
const ProfileSection: React.FC<ProfileSectionProps> = ({ 
  currentUser, formData, isSaving, onFormChange, onSubmit 
}) => {
    ...
    <form onSubmit={onSubmit} className="bg-white dark:bg-slate-900/60 rounded-[2.5rem] border p-8 md:p-12 shadow-sm space-y-10 glow-card animate-in slide-in-from-right-4">
      <div className="flex flex-col md:flex-row items-center gap-8">
        <div className="relative group">
          <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-slate-50 dark:border-slate-800 shadow-xl relative">
            <img src={currentUser.profilePhoto} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt="Profile" />
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-pointer"><Camera className="text-white" size={24} /></div>
          </div>
          <button type="button" className="absolute -bottom-1 -right-1 bg-primary text-white p-2.5 rounded-2xl shadow-lg border-2 border-white dark:border-slate-900 hover:scale-110 transition-transform"><Plus size={16} /></button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-2">
          ...
          <input type="email" value={formData.email} onChange={(e) => onFormChange({ email: e.target.value })} className="w-full bg-slate-50 dark:bg-slate-800/50 border rounded-2xl px-5 py-3.5 text-sm focus:ring-2 focus:ring-primary/20 outline-none" />
        </div>

...
import { useMuseumExhibits } from '../hooks/useMuseum';
import type { ExhibitData } from '../types';

export default function MuseumPage() {
  const { data: exhibits } = useMuseumExhibits();
    ...
    <div className="w-screen h-screen bg-[#0E0C0B] overflow-hidden fixed inset-0 z-50 text-[var(--clr-linen)] font-ui">
      ...
      <Suspense fallback={<div className="flex items-center justify-center h-full w-full"><p className="font-script text-[48px] text-[var(--clr-gold)] animate-pulse">"Lighting the gallery..."</p></div>}>
        <Canvas camera={{ position: [0, 0, 5], fov: 45 }}>
          ...
          <ScrollControls pages={4} damping={0.25} distance={1}>
            <MuseumHall exhibits={exhibits} onDecadeChange={setActiveDecade} onSelectExhibit={setSelectedExhibit} />
          </ScrollControls>
        </Canvas>
      </Suspense>
      ...
      <div className="absolute top-8 left-8 z-10 pointer-events-none">
        <AnimatePresence mode="wait">
          <motion.div key={activeDecade} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 0.9, y: 0 }} exit={{ opacity: 0, y: 10 }} transition={{ duration: 0.4 }}>
      ...
      <div className="absolute bottom-8 w-full flex justify-center z-10 pointer-events-none">
        <div className="bg-[rgba(20,18,17,0.82)] backdrop-blur-md border border-[rgba(184,143,91,0.2)] rounded-full px-4 py-2 flex gap-2 pointer-events-auto shadow-2xl">
          {['1950s', '1960s', '1970s', '1980s'].map(dec => (
            <button key={dec} className={`px-4 py-2 rounded-full text-[11px] font-bold uppercase tracking-widest transition-colors ${activeDecade === dec ? 'bg-[rgba(184,143,91,0.15)] text-[var(--clr-gold)]' : 'text-[var(--clr-fog)] hover:text-[var(--clr-linen)]'}`}>
              {dec}
            </button>
          ))}
        </div>
      ...
      <AnimatePresence>
        {selectedExhibit && !isDeepDiveOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, backdropFilter: 'blur(0px)' }} animate={{ opacity: 1, backdropFilter: 'blur(12px)' }} exit={{ opacity: 0, backdropFilter: 'blur(0px)' }} transition={{ duration: 0.4 }} className="absolute inset-0 bg-[rgba(14,12,11,0.7)]" onClick={() => setSelectedExhibit(null)} />
            <motion.div initial={{ opacity: 0, y: 40, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} className="relative w-full max-w-[480px] bg-[var(--clr-charcoal)] border border-[rgba(184,143,91,0.35)] rounded-[var(--radius-lg)] shadow-[var(--shadow-lg)] overflow-hidden flex flex-col z-10">
              ...
              <div className="p-8 relative -mt-10">
                ...
                <p className="font-ui text-[11px] uppercase tracking-widest text-[var(--clr-gold)] mb-6 drop-shadow-md">{selectedExhibit.location} &middot; {selectedExhibit.year}</p>

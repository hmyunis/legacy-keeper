...
import { UserFocus, Palette, Database, MagicWand, Trash, Warning, Sparkle } from '@phosphor-icons/react';
import { sileo } from 'sileo';
import { Button } from '../../../components/ui/Button';
import { motion } from 'framer-motion';
import { usePurgeStorage, useUpdateProfile } from '../hooks/useSettings';
...
export default function SettingsPage() {
  ...
  const purgeMutation = usePurgeStorage();
  const profileMutation = useUpdateProfile();

  const handleSmartPurge = () => {
    sileo.promise(purgeMutation.mutateAsync(), {
      loading: { title: "Scanning", description: "AI curator is analyzing redundancy..." },
      success: { title: "Culling Complete", description: "142 MB reclaimed from the vault." },
      error: { title: "Error", description: "Optimization failed." }
    });
  };

  const handleSaveAccount = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    sileo.promise(profileMutation.mutateAsync({}), {
      loading: { title: "Saving..." },
      success: { title: "Identity Updated", description: "Your profile has been saved." },
      error: { title: "Error", description: "Could not save profile." }
    });
  };
            ...
            {activeTab === 'HEALTH' && (
              <div className="space-y-10 animate-in fade-in duration-500">
                ...
                <div className="pt-8 border-t border-[var(--clr-aged)] flex justify-between items-center">
                   ...
                   <Button variant="primary" onClick={handleSmartPurge} disabled={purgeMutation.isPending}>
                     {purgeMutation.isPending ? 'PURGING...' : 'START SMART PURGE'}
                   </Button>
                </div>
              </div>
            )}
            ...
            {activeTab === 'ACCOUNT' && (
              <form onSubmit={handleSaveAccount} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="pt-6 border-t border-[var(--clr-aged)]">
                  <Button variant="primary" disabled={profileMutation.isPending}>
                    {profileMutation.isPending ? 'SAVING...' : 'SAVE IDENTITY'}
                  </Button>
                </div>
              </form>
            )}

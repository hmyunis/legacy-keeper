...
const TIMELINE_DATA = [
  ...
  { id: '3', decade: '1980s', year: '1982', title: 'Wedding Anniversary', location: 'Entoto', desc: 'Celebrating 20 years of marriage. The whole family gathered for a massive feast.', url: 'https://images.unsplash.com/photo-1511895426328-dc8714191300?q=80&w=600' },
  { id: '4', decade: '1980s', year: '1989', title: 'University Graduation', location: 'Dire Dawa', desc: 'Yohannes graduates from engineering school with honors.', url: 'https://images.unsplash.com/photo-1582213782179-e0d53f98f2ca?q=80&w=600' },
];
...
export default function Timeline() {
  ...
  const filteredData = activeFilter === 'All Eras'
    ? TIMELINE_DATA
    : TIMELINE_DATA.filter(item => item.decade === activeFilter);

  const groupedData = filteredData.reduce((acc, item) => {
    if (!acc[item.decade]) acc[item.decade] = [];
    acc[item.decade].push(item);
    return acc;
  }, {} as Record<string, typeof TIMELINE_DATA>);

...
import { TreeCanvas, type TreeNode, type TreeEdge } from '../features/family-tree/TreeCanvas';
...
// Richly animated background layer for the botanical tree feel
...
export default function FamilyTree() {
  ...
  const handleAddRelative = (parentId: string) => {
    sileo.info({ title: "Preparing Lineage Registry", description: `Adding a new descendant for ${TREE_NODES.find(n => n.id === parentId)?.name || 'the family'}.` });
  };
  ...
  return (
    <div className="w-screen h-screen bg-[var(--clr-parchment)] relative overflow-hidden flex flex-col zone-light">
      ...
      <div
        aria-hidden
        className="lineage-tree-watermark absolute inset-0 pointer-events-none z-[1] flex items-center justify-center opacity-[0.055]"
      >
      ...
      {/* Free drag bounds implemented cleanly utilizing massive transparent background to absorb native dragging events */}
      <div className="absolute inset-0 z-10 overflow-hidden cursor-grab active:cursor-grabbing">
        <motion.div
          ...
          dragElastic={0.3}
          animate={{ scale }}
          className="absolute w-[6000px] h-[6000px] left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 origin-center bg-black/0"
        >
          <TreeCanvas
            nodes={TREE_NODES}
            edges={TREE_EDGES}
      ...
      <div className="absolute bottom-12 left-12 z-20 flex gap-4 pointer-events-auto">
        <button
          onClick={() => {
            ...
            sileo.info({
              title: isEditMode ? "Lineage locked" : "Grafting points revealed",
              icon: isEditMode ? <X /> : <TreeStructure weight="fill" />
            });
          }}
          className={`flex items-center gap-3 px-8 py-4 rounded-full font-ui font-bold text-[11px] uppercase tracking-[0.15em] shadow-xl transition-all ${
            isEditMode ? 'bg-[var(--clr-charcoal)] text-[var(--clr-gold)]' : 'bg-[var(--clr-gold)] text-[var(--clr-charcoal)] hover:bg-[var(--clr-gold-light)]'
          }`}
        >
      ...
      <div className="absolute bottom-12 right-12 z-20 flex flex-col gap-3 pointer-events-auto">
        <button onClick={() => setScale(s => Math.min(s + 0.2, 2))} className="w-12 h-12 rounded-full bg-[rgba(247,244,239,0.9)] backdrop-blur-md border border-[var(--clr-aged)] text-[var(--clr-ink)] flex items-center justify-center hover:text-[var(--clr-gold)] hover:border-[var(--clr-gold)] shadow-[var(--shadow-sm)] transition-all">
          <MagnifyingGlassPlus size={20} weight="bold" />
        </button>
        <button onClick={() => setScale(s => Math.max(s - 0.2, 0.4))} className="w-12 h-12 rounded-full bg-[rgba(247,244,239,0.9)] backdrop-blur-md border border-[var(--clr-aged)] text-[var(--clr-ink)] flex items-center justify-center hover:text-[var(--clr-gold)] hover:border-[var(--clr-gold)] shadow-[var(--shadow-sm)] transition-all">
          <MagnifyingGlassMinus size={20} weight="bold" />
        </button>
        <button onClick={() => setScale(1)} className="w-12 h-12 rounded-full bg-[rgba(247,244,239,0.9)] backdrop-blur-md border border-[var(--clr-aged)] text-[var(--clr-ink)] flex items-center justify-center hover:text-[var(--clr-gold)] hover:border-[var(--clr-gold)] shadow-[var(--shadow-sm)] transition-all">
      ...
      <AnimatePresence>
        {selectedNode && (
          <motion.div
            ...
            className="absolute top-0 right-0 h-full w-[440px] bg-[var(--clr-charcoal)] border-l border-[var(--clr-gold)] shadow-[-20px_0_60px_rgba(20,18,17,0.5)] z-[60] flex flex-col"
          >
            <div className="relative h-[240px] w-full flex items-center justify-center bg-[var(--clr-soot)] border-b border-[rgba(184,143,91,0.2)] overflow-hidden">
              {selectedNode.photo && (
                <div className="absolute inset-0 opacity-20 blur-md pointer-events-none">
                  <img src={selectedNode.photo} className="w-full h-full object-cover" alt="" />
                </div>
              )}
              <button onClick={() => setSelectedNodeId(null)} className="absolute top-8 right-8 w-10 h-10 rounded-full border border-[var(--clr-gold)] text-[var(--clr-gold)] flex items-center justify-center hover:bg-[var(--clr-gold)] hover:text-[var(--clr-charcoal)] transition-colors z-10">
            ...
            <div className="flex-1 p-10 overflow-y-auto no-scrollbar text-center text-[var(--clr-linen)]">
              ...
              <div className="space-y-4 mb-10 text-left px-4">
                <div className="flex justify-between border-b border-[rgba(184,143,91,0.15)] pb-3">
                  <span className="font-ui text-[11px] uppercase tracking-[0.2em] text-[var(--clr-fog)]">Born</span>
                  <span className="font-ui text-[13px] text-[var(--clr-linen)]">1942 · Harar</span>
                </div>
                {selectedNode.deathYear && (
                  <div className="flex justify-between border-b border-[rgba(184,143,91,0.15)] pb-3">
                    <span className="font-ui text-[11px] uppercase tracking-[0.2em] text-[var(--clr-fog)]">Passed</span>
                    <span className="font-ui text-[13px] text-[var(--clr-linen)]">{selectedNode.deathYear}</span>
                  </div>
                )}
              </div>

              <div className="space-y-4 w-full">
                <Button variant="primary" onClick={handleAction} className="w-full py-4 text-[10px]">
                  <ImageIcon size={18} /> View Exhibition
                </Button>
                <Button variant="ghost" onClick={handleAction} className="w-full py-4 text-[10px]">
                  <MagicWand size={18} /> Generate Chronicle
                </Button>
              </div>
            </div>
          </motion.div>

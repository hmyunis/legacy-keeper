...
import { UsersThree, UserPlus, LinkBreak, ShieldCheck, ArrowsMerge, X } from '@phosphor-icons/react';
...
export default function Members() {
  ...
  const handleInviteSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      sileo.success({ title: "Invitation Sent", description: "Registry keys sent to kin's email." });
      setIsSubmitting(false);
      setShowInvite(false);
    }, 1200);
  };

  const handlePactRequest = () => {
    setIsSubmitting(true);
    setTimeout(() => {
      sileo.success({ title: "Pact Dispatched", description: "Requesting lineage federation." });
      setIsSubmitting(false);
  ...
  const handleRemoveMember = (name: string) => {
    sileo.error({ title: "Access Revoked", description: `Keys disabled for ${name}.` });
  };
            ...
            <div className="fixed inset-0 z-[110] flex items-center justify-center p-6">
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-lg bg-[var(--clr-parchment)] border-2 border-[var(--clr-gold)] rounded-[var(--radius-lg)] p-10 shadow-2xl">
                ...
                <form onSubmit={handleInviteSubmit} className="space-y-4">
                  <input required type="text" placeholder="Relative's Full Name" className="w-full bg-[var(--clr-linen)] border border-[var(--clr-aged)] rounded-full px-6 py-4 outline-none focus:border-[var(--clr-gold)]" />
                  <input required type="email" placeholder="Email Address" className="w-full bg-[var(--clr-linen)] border border-[var(--clr-aged)] rounded-full px-6 py-4 outline-none focus:border-[var(--clr-gold)]" />
                  <select className="w-full bg-[var(--clr-linen)] border border-[var(--clr-aged)] rounded-full px-6 py-4 outline-none focus:border-[var(--clr-gold)] appearance-none">
            ...
            <div className="fixed inset-0 z-[110] flex items-center justify-center p-6">
              ...
              >
                <div className="text-center space-y-6">
                  ...
                  <input type="email" placeholder="Partner's Email Address" className="w-full bg-[var(--clr-linen)] border border-[var(--clr-aged)] rounded-full px-6 py-4 outline-none focus:border-[var(--clr-gold)]" />
                  ...
                  <Button variant="primary" className="w-full" onClick={handlePactRequest} disabled={isSubmitting}>
                    {isSubmitting ? 'PROCESSING...' : 'SEND PACT REQUEST'}
                  </Button>
                </div>
              </motion.div>
            </div>
